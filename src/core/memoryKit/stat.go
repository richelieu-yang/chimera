...
(1) Alloc: is bytes of allocated heap objects.
	分配的堆对象的字节数。

(2) TotalAlloc: is cumulative bytes allocated for heap objects.
	堆对象分配的累积字节数。与 Alloc 不同，当对象被释放时它不会减少。

(3) Sys: is the total bytes of memory obtained from the OS.
	从操作系统获得的内存总字节数。这测量了 Go 运行时为堆、栈和其他内部数据结构保留的虚拟地址空间。

(4) NumGC: is the number of completed GC cycles.
