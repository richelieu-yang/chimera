...
func (client Client) Set(key string, value interface{}, expiration time.Duration) (bool, error) {
	reply, err := client.UC.Set(context.Background(), key, value, expiration).Result()
	if err != nil {
		return false, err
	}
	return reply == "OK", nil
...
func (client Client) SetNX(key string, value interface{}, expiration time.Duration) (bool, error) {
	return client.UC.SetNX(context.Background(), key, value, expiration).Result()
}
...
func (client Client) Get(key string) (string, error) {
	return client.UC.Get(context.Background(), key).Result()
}
