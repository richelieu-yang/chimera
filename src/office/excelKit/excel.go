...
func ConvertAxisToRowCol(axis string) (int, int, error) {
	...
	if !re.MatchString(axis) {
		return 0, 0, errorKit.Simple("axis(%s) is invalid", axis)
	}
...
//	row--
//	if row < 0 || row > MaxRow {
//		return 0, errorKit.Simple("row(%d) is invalid", row)
//	}
//	return row, nil
//}
//
...
func ConvertRowToString(row int) (string, error) {
	if row < 0 || row > MaxRow {
		return "", errorKit.Simple("row(%d) is invalid", row)
	}
...
func ConvertColToString(col int) (string, error) {
	...
	if col < 0 || col > MaxCol {
		return "", errorKit.Simple("col(%d) is invalid", col)
	}
