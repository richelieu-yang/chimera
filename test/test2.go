...
func main() {
	...
	//if err := jsonKit.UnmarshalFromString(str, &in); err != nil {
	//	panic(err)
	//}

	in := []string{"1", "0", "2"}

	if err := yamlKit.MarshalToFileWithJsonTag1(in, "a.yaml"); err != nil {
		panic(err)
	}
	if err := yamlKit.MarshalToFileWithJsonTag(in, "b.yaml"); err != nil {
		panic(err)
	}
